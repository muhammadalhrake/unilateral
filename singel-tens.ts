...
export function generateSingelTens(singelTensProbabilityPyramid:stateOfSingelTens, stat: string) {
  let copyOfMonad = {
    answers: [5, 5, 5, 5],
    firstNumber: 5,
    secondNumber: 5
  };
  if (singelTensProbabilityPyramid.EZ.length == 0 || singelTensProbabilityPyramid.Hard.length == 0) {
  ...
  if (stat == 'Easy') {
    ...
    singelTensProbabilityPyramid.EZ=filterA(singelTensProbabilityPyramid.EZ,mumber)
    ...
    copyOfMonad.secondNumber = +question[1];
    copyOfMonad.answers = ansArray(+question[0], +question[1]);
    singelTensProbabilityPyramid.Hard=filterA(singelTensProbabilityPyramid.Hard,mumber)
    return copyOfMonad;
  }
}

function generateProbabilitySpace(singelTensProbabilityPyramid:stateOfSingelTens, stat: string) {
  for (let i = 0; i <= 9; i++) {
    for (let j = 0; j <= 9; j++) {
